...
**Status:** Proposal for maintainer sign-off. Nothing here is implemented.
**Tracking issue:** https://github.com/SpillwaveSolutions/agent-memory/issues/46 (canonical copy; decisions go in its comments)
...
**Acceptance:**
- [ ] A tag on a commit not in `main` fails at `verify` before any build
      starts (test with `v0.0.0-guardtest` on a throwaway branch; delete it)
...
**Acceptance:**
- [ ] `docs/plans/march-branch-triage.md` lists every ported file with
...
1. **March branch line** — port export/import + Claude registration by
   feature (recommended), rewrite from scratch, or drop?
2. **Launch timing** — blog now, product posts after 60-02 (recommended),
   or hold everything?
3. **Daemonization** — service unit files (recommended; survives reboot,
   matches the guidance the daemon already prints) or true double-fork?
4. **Backfill mode** — stopped-daemon CLI only for v3.2 (recommended), or
   also an admin RPC against a running daemon?
