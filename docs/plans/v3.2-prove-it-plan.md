...
v3.1 was re-planned. Two sibling branches
(`gsd/phase-57-opencode-converter-registration`,
`gsd/phase-58-claude-registration-metadata`) are also unmerged; the OpenCode
one is now obsolete since OpenCode was removed in Phase 57.
...
- Produce a one-page inventory of the three `gsd/` branches: what each
  ...
  still wanted
- Recommendation going in: **cherry-pick the export/import feature set by
  feature** (JSONL backup, import/bootstrap, streaming RPCs) rather than
  merge 81 commits, and use the import path as the foundation for 61-01
- Delete `gsd/phase-57-opencode-converter-registration` (obsolete)
...
### Phase 61 — Operate It  (4 plans · ~5 sessions)
...
on the 59-02 decision. Roughly **12–14 agent sessions plus one maintainer
...
1. **Export/import branch:** cherry-pick by feature, rewrite from scratch,
   or drop? (Recommendation: cherry-pick by feature; it feeds 61-01.)
