...
Invoke this agent (`@memory-navigator`) for complex queries that benefit from intelligent routing. OpenCode does not support automatic trigger patterns, so use explicit `@memory-navigator` invocation for these query types:
...
Since OpenCode agents do not support automatic triggers, use `@memory-navigator` when a query matches these patterns:
...
**Tip:** Any query about past conversations, previous sessions, or recalling what was discussed should be directed to `@memory-navigator`.
