...
must_haves:
  ...
  artifacts:
    ...
    - path: "crates/memory-bench/src/scorer.rs"
      provides: "Result scoring: accuracy, recall@k, latency percentiles"
      exports: ["BenchmarkReport", "score_result", "compute_accuracy"]
...
7. Run: `cargo clippy -p memory-bench -- -D warnings` — no warnings.
  ...
  <acceptance_criteria>
    ...
    - cargo test -p memory-bench passes (10+ tests including scorer, baseline, report)
    - cargo clippy -p memory-bench -- -D warnings passes
  </acceptance_criteria>
  <done>Runner, scorer, report, and baseline modules exist with passing tests. Scorer computes accuracy/recall/percentiles. Report generates JSON and markdown. Baseline loads competitor TOML.</done>
</task>
