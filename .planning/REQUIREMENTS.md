...
- [x] **CANON-02**: Canonical hook definitions in YAML format capture all event types across runtimes *(reinterpreted: hooks generated per-converter in `generate_guidance` — YAML canonical format unnecessary since each runtime's hook mechanism is radically different)*
...
- [x] **MIG-03**: Old adapter directories archived with README stubs pointing to `memory-installer`
...
| CANON-02 | Phase 49 | Complete (reinterpreted) |
